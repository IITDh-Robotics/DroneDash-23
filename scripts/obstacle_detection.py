import rospy
from geometry_msgs.msg import PoseStamped
from mavros_msgs.msg import State
from mavros_msgs.srv import CommandBool, CommandBoolRequest, SetMode, SetModeRequest
from px4_msgs.msg import ObstacleDistance

current_state = State()
obstacle_distance = ObstacleDistance()
obstacle_detected = False

def state_cb(msg):
    global current_state
    current_state = msg

def obstacle_distance_cb(msg):
    global obstacle_distance
    obstacle_distance = msg

def detect_obstacle():
    global obstacle_detected
    if obstacle_distance.min_distance < 1.0:
        obstacle_detected = True
    else:
        obstacle_detected = False

if __name__ == "__main__":
    rospy.init_node("obs_node_py")

    state_sub = rospy.Subscriber("mavros/state", State, callback=state_cb)
    obstacle_distance_sub = rospy.Subscriber("mavros/obstacle_distance", ObstacleDistance, callback=obstacle_distance_cb)

    local_pos_pub = rospy.Publisher("mavros/setpoint_position/local", PoseStamped, queue_size=1)

    rospy.wait_for_service("/mavros/cmd/arming")
    arming_client = rospy.ServiceProxy("mavros/cmd/arming", CommandBool)

    rospy.wait_for_service("/mavros/set_mode")
    set_mode_client = rospy.ServiceProxy("mavros/set_mode", SetMode)

    rate = rospy.Rate(20)

    while not rospy.is_shutdown() and not current_state.connected:
        rate.sleep()

    pose = PoseStamped()
    pose.pose.position.x = 0
    pose.pose.position.y = 0
    pose.pose.position.z = 2

    # Send a few setpoints before starting
    for i in range(100):
        if rospy.is_shutdown():
            break
        local_pos_pub.publish(pose)
        rate.sleep()

    offb_set_mode = SetModeRequest()
    offb_set_mode.custom_mode = 'OFFBOARD'

    arm_cmd = CommandBoolRequest()
    arm_cmd.value = True

    last_req = rospy.Time.now()

    

    while not rospy.is_shutdown():
        # First set the mode to offboard (refer to PX4 Flight Modes)
        if current_state.mode != "OFFBOARD" and (rospy.Time.now() - last_req) > rospy.Duration(5.0):
            if set_mode_client.call(offb_set_mode).mode_sent == True:
                rospy.loginfo("OFFBOARD enabled")
            last_req = rospy.Time.now()

        # Arm the vehicle
        elif not current_state.armed and (rospy.Time.now() - last_req) > rospy.Duration(5.0):
            if arming_client.call(arm_cmd).success == True:
                rospy.loginfo("Vehicle armed")
            last_req = rospy.Time.now()

        # Move the vehicle
        elif current_state.armed and (rospy.Time.now() - last_req) > rospy.Duration(5.0):
            # Check for obstacles
            detect_obstacle()
            
            if obstacle_detected:
                # Stop forward movement
                pose.pose.position.x = 0
                rospy.loginfo("Obstacle detected! Stopping forward movement")
            else:
                # No obstacles detected, continue with default movement behavior
                if pose.pose.position.x == 0:
                    pose.pose.position.x = 2  # If x setpoint is 0, change it to 2
                    rospy.loginfo("Moving forward")
                else:
                    pose.pose.position.x = 0  # If x setpoint is 2, change it to 0
                    rospy.loginfo("Moving backward")
                    
            last_req = rospy.Time.now()

        local_pos_pub.publish(pose)
        rate.sleep()
